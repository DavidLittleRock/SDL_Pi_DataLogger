...
#             ax.axis([max(fds)-days, max(fds), 10, 110])
 ...
 #           ax2.legend(loc='upper right')
            ax2.axis([max(fds)- days, max(fds), 10, 110])
...
def buildWindGraph(username, password, myGraphSampleCount):
        dz = datetime.now()
        print('buildWindGraph - The time is: %s' % datetime.now())
        # open database
        ...
        ax.legend(loc='upper left')
        ax2.legend(loc='upper right')
        ax.axis([max(fds)-2, max(fds), 0, max(gustSG)+1])
        ax2.axis([max(fds)-2, max(fds), 0, max(gustSG)+1])
...
#          query = '(SELECT timestamp, deviceid, Barometric_Pressure, id FROM '+dataBaseTable+' ORDER BY id DESC LIMIT '+ str(myGraphSampleCount) + ') ORDER BY id ASC' 
         ...
         ax.axis([min(fds), max(fds), minPres, maxPres])
...
#        print (myGraphSampleCount)
        ...
        ax.legend(loc='upper left')
        ax.axis([min(fds), dates.date2num(datetime.now()), 0, max(rainTotal)+0.1])
 #       ax.axis([min(fds), max(fds), 0, max(rainTotal)+0.1])
        ...
        ax2.legend(loc='upper right')
        ax2.axis([max(fds)- days, dates.date2num(datetime.now()), 0, max(rainTotal)+0.1])
