...
def buildWindGraph(username, password, myGraphSampleCount):
    ...
    ax.xaxis.set_major_formatter(hfmt)
    ax.plot(fds, windSpeedSG, color='red',label='Wind Speed (mph)' ,linestyle='solid',marker=".")
    ax.set_xlabel('Time')
...
def buildRainGraph(username, password, myGraphSampleCount):
    ...
    for days in daysToPrint:
        ...
        query = '(SELECT id, timestamp, Rain_Change FROM RainPeriod WHERE DATE(timestamp)=Current_Date-'+str(days)+' LIMIT '+ str(myGraphSampleCount) + ') ORDER BY id ASC' 
        ...
        for record in result:
            ...
            rainPeriodA.append(record[2]/22.5) 
    ...
    ax.xaxis.set_minor_locator(dates.HourLocator(byhour=(6,12,18)))
    ax.plot(fds, rainPeriod, color='red',label="Rain this period ",linestyle="None",marker=".") # x=fds(the matplotlib date number), y= u(temperature)
    ax.set_xlabel("Time")
    ...
    ax2.legend(loc='upper right')
    ax2.axis([dates.date2num(datetime.now())-6, dates.date2num(datetime.now()), 0, max(rainTotal)+0.1])
